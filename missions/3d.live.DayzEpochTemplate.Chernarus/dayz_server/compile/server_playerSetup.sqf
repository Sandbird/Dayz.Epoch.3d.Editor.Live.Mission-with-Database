private ["_characterID","_playerObj","_playerID","_dummy","_worldspace","_state","_doLoop","_key","_primary","_medical","_stats","_humanity","_lastinstance","_friendlies","_randomSpot","_position","_debug","_distance","_hit","_fractures","_score","_findSpot","_pos","_isIsland","_w","_clientID","_spawnMC","_namespace","_date"];
...
//diag_log format["PLAYER_SETUP: _characterID:%1 _playerObj:%2 _playerID:%3",_characterID,_playerObj,_playerID];
...
_key = format["SELECT `PlayerUID`, `Medical`, `Worldspace`, `Generation`, `KillsZ`, `HeadshotsZ`, `KillsH`, `KillsB`, `CurrentState`, `Humanity`, `InstanceID` FROM `Character_DATA` WHERE `CharacterID`='%1'",_characterID];
_primary = _key call server_hiveReadWrite;
_primary = _primary select 0 select 0;
//diag_log format["PRIMARY:%1",_primary];
...
_medical 		=	call compile (_primary select 1);
_worldspace = call compile (_primary select 2);
_humanity 	=	call compile (_primary select 9);
_lastinstance =	call compile (_primary select 10);

_stats = [];
_stats = call compile format["[%1,%2,%3,%4]", (_primary select 4),(_primary select 5),(_primary select 6),(_primary select 7)];
_state =	call compile (_primary select 8);
...
dayzPlayerLogin2 = [_worldspace,_state,_randomSpot]; // Sandbird
