...
if (_characterID != "0") then {
	...
	if (_characterID != "0") then {		
		...
		*/
		//`HeadshotsZ` = '0',`KillsZ` = '0',`KillsH` = '0',`KillsB` = '0',`Humanity` = '0', `DistanceFoot` = '3852', `Model` = '"Survivor2_DZ"',
		...
		*/
		...
		if (!isNull _character) then {
			if (alive _character) then {
				...
				_key call server_hiveWrite;
				sleep 0.5;
				_key = format["UPDATE `character_data` SET `Inventory` = '%1',`Backpack` = '%2',`Worldspace` = '%3',`Medical` = '%4',`CurrentState` = '%5',`HeadshotsZ` = '%6',`KillsZ` = '%7',`KillsH` = '%8',`KillsB` = '%9',`Humanity` = '%10', `DistanceFoot` = '%11', `Model` = '%12', `Duration` = '%13' WHERE `character_data`.`CharacterID` = '%14'",_playerGear, _playerBackp, _playerPos, _medical, _currentState,_headShots, _kills, _killsH,_killsB, _humanity, _distanceFoot, (typeOf _character), _timeSince, _characterID];
				_key call server_hiveWrite;
			};
		};
