...
if (!DefaultTruePreMadeFalse) then {
		_key = format["SELECT COUNT(*) FROM object_data WHERE `Instance`= '%1'",dayZ_instance];
		_hiveResponse = _key call server_hiveReadWrite;  
};
		diag_log ("HIVE: found "+str(_hiveResponse select 0 select 0)+" objects" );
		_i = 99; // break
	...
	_objectQueue = [];
		...
		// save superkey
		profileNamespace setVariable ["SUPERKEY","DemoSuperKey"];
...
if (!DefaultTruePreMadeFalse) then {
	...
	_key2 		= format["SELECT COUNT(*) FROM object_data WHERE `Instance`='%1'",dayZ_instance];
	_data2 = _key2 call server_hiveRead;
if (!DefaultTruePreMadeFalse) then {
	_objectCount = parseNumber(_data2 select 0 select 0);
}else{
	_objectCount = 0;
};
		...
		for "_i" from 0 to (_objectCount - 1) do {
			...
			sleep .001;
			//diag_log (format["HIVE dbg %1 %2", typeName _response, _response]);
			if ((_response select 1) isKindOf "ModularItems") then {
