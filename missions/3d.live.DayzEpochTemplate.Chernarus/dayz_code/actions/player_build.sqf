...
if (_hasrequireditem) then {
	...
	while {_isOk} do {
		...
		if(_location1 distance _location2 > 5) exitWith {
			_isOk = false;
			_cancel = true;
			_reason = "Youve moved to far away from where you started building (within 5 meters)";
			detach _object;
			deleteVehicle _object;
		};
			...
			if(_num_removed == 1) then {
				...
				if(_lockable > 1) then {
					...
					if(_tmpbuilt isKindOf "Land_Fire_DZ") then {
						...
						publicVariableServer "PVDZE_obj_Publish";
						[dayz_characterID,_tmpbuilt,[_dir,_location],_classname] call server_publishObj;
					};
				};
			} else {
