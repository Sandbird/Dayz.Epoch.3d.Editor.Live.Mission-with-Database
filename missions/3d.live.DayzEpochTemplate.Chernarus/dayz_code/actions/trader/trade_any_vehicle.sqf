...
if (_finished) then {
	...
	if (_canAfford) then {
		...
		if(_bos == 1) then {
			...
			[_activatingPlayer,_traderID,_bos,_part_in,inTraderCity,_part_out,_qty_out] spawn server_tradeObj;
		} else {
			// Buying
			PVDZE_obj_Trade = [_activatingPlayer,_traderID,_bos,_part_out,inTraderCity,_part_in,_qty_in];
			[_activatingPlayer,_traderID,_bos,_part_out,inTraderCity,_part_in,_qty_in] spawn server_tradeObj;
		};
			...
			if (_qty >= _qty_in) then {
				...
				if(local _obj and !isNull _obj and alive _obj and !_notSetup) then {
					...
					if(_okToSell) then {
						...
						[_objectID,_objectUID,_activatingPlayer] spawn server_deleteObj;
