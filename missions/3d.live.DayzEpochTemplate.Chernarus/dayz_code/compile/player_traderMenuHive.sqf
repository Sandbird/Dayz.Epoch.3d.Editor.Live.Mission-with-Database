...
TraderDialogLoadItemList = {
	...
	if(isNil "PVDZE_plr_TradeMenuResult") then {
		...
		publicVariable  "PVDZE_plr_TradeMenu";
		[_activatingPlayer,_trader_id] spawn server_traders;
		waitUntil {!isNil "PVDZE_plr_TradeMenuResult"};
		
		//diag_log format[">>>>>>>>>> %1",PVDZE_plr_TradeMenuResult];
	...
	_item_list = [];
	{
	 ...
	 // call compile format ["%1",_x];
		...
		_afile = _x select 7;
		_File = "dayz_code\actions\trader\" + _afile + ".sqf";
		...
		_index = lbAdd [TraderDialogItemList, format["%1 (%2) [Qt:%3]", _textPart, _name, _qty]];
