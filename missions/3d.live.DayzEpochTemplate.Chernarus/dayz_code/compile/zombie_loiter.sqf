...
if (count _this > 2) then {
	...
	if ((_chance % 4) == 0) then {
		...
		_pos = [_playerpos,30,120,4,0,5,0] call fnc_findSafePos;
	} else {
	_pos = [_originalPos,10,90,4,0,5,0] call fnc_findSafePos;
	};
	
	if (_unit distance player > 250) then {
		_pos = [_playerpos,120,200,4,0,5,0] call fnc_findSafePos;
	};
};
