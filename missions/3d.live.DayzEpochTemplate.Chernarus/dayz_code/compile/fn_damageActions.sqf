...
if (!isNull _menClose and _hasPatient and !r_drag_sqf and !r_action and !_inVehicle and !r_player_unconscious and _isClose) then {
	_unit = 		cursorTarget;
	_isDisallowRefuel = typeOf _unit in ["WarfareBMGNest_M240_US_EP1"];
	// player reveal _unit;
	...
	_charID =		  player getVariable ["CharacterID", 0];  //getPlayerUID _unit;
	...
	if ((_unit isKindOf "AllVehicles") and !(_unit isKindOf "Man") and !_isDisallowRefuel) then {
		...
		if ((isPlayer _unit) and !(_charID in _friendlies)) then {
			r_action = true;
			_action = _unit addAction ["Tag as friendly", "dayz_code\actions\player_tagFriendly.sqf", [], 0, false, true, "", ""]; //\z\addons\
			r_player_actions set [count r_player_actions,_action];
		};
