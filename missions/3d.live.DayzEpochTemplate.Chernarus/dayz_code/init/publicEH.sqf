...
if (isServer) then {
	...
	if!(DefaultTruePreMadeFalse) then {
		//"PVDZE_plr_Login"		addPublicVariableEventHandler {_id = (_this select 1) spawn sdbd_fnc_login};
		"PVDZE_plr_Login"		addPublicVariableEventHandler {_id = (_this select 1) spawn server_playerLogin};
		//"PVDZE_plr_Login2"		addPublicVariableEventHandler {(_this select 1) call sdbd_fnc_setup};
	...
	"PVDZA_boobyTrap" addPublicVariableEventHandler {[_this select 1] execVM 'custom\bmb\pbb.sqf'};
	
	//"PVDZE_SpawnLight"	addPublicVariableEventHandler {(_this select 1) execVM 'custom\spawnlight.sqf'};
	//Area Maintenance
	"maintainArea_log"		addPublicVariableEventHandler {
		_val = _this select 1;
		_player = _val select 0;
		_playerName = name _player;
		_playerID = _player getVariable ["playerUID", "0"]; //getPlayerUID _player;
