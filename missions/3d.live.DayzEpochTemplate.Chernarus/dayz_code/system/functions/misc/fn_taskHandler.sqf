...
if (isServer && !(local (_this select 0))) then {
	[(_this select 0), (_this select 1)] spawn {
		...
		if ((_this select 0) call BIS_checkAITasks) then {(_this select 0) setVariable ["BIS_tasksPriorityLadder", (_this select 1), TRUE]; BIS_AITasks = (_this select 0) execFSM "ca\modules\functions\misc\fn_AITasks.fsm"}
	}
};
...
if ((_this select 0) call BIS_checkAITasks) then {
	(_this select 0) setVariable ["BIS_tasksPriorityLadder", (_this select 1), TRUE]; BIS_AITasks = (_this select 0) execFSM "ca\modules\functions\misc\fn_AITasks.fsm"
} else {
	_this spawn {
